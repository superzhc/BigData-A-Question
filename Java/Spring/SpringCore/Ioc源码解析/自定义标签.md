...
- [自定义标签](#自定义标签)
  - [使用自定义标签](#使用自定义标签)
    - [1、创建组件](#1创建组件)
    - [2、定义 XSD 文件](#2定义-xsd-文件)
    - [3、Parser 类](#3parser-类)
    - [4、Handler 类](#4handler-类)
    - [5、Spring.handlers](#5springhandlers)
    - [6、Spring.schemas](#6springschemas)
    - [使用示例](#使用示例)
  - [解析自定义标签](#解析自定义标签)
...
5. 编写 Spring.handlers 和 Spring.schemas 文件【**文件所在的位置在`META-INF`文件夹下**】
...
这个处理过程很简单明了，根据 namespaceUri 获取 Handler，这个映射关系在 Spring.handlers 中已经定义了，所以只需要找到该类，然后初始化返回，最后调用该 Handler 对象的 `parse()` 方法处理，该方法编写的 Parser 类也提供了实现。所以上面的核心就在于怎么找到该 Handler 类。调用方法为：`this.readerContext.getNamespaceHandlerResolver().resolve(namespaceUri)`
...
`init()` 方法主要是将自定义标签解析器进行注册，如编写的 Handler 类自定义的 `init()` ：
...
核心在方法 parseInternal() 为什么这么说，因为该方法返回的是 AbstractBeanDefinition 对象，从前面默认标签的解析工作中我们就可以判断该方法就是将标签解析为 AbstractBeanDefinition ，且后续代码都是将 AbstractBeanDefinition 转换为 BeanDefinitionHolder，所以真正的解析工作都交由 parseInternal() 实现，如下：
...
在该方法中主要关注两个方法：`getBeanClass()` 、`doParse()`。对于 `getBeanClass()` 方法，AbstractSingleBeanDefinitionParser 类并没有提供具体实现，而是直接返回 null，意味着它希望子类能够重写该方法，当然如果没有重写该方法，这会去调用 `getBeanClassName()` ，判断子类是否已经重写了该方法。对于 `doParse()` 则是直接空实现。所以对于 `parseInternal()` 而言它总是期待它的子类能够实现 `getBeanClass()`、`doParse()`，其中 `doParse()` 尤为重要，如果不提供实现就无法解析自定义标签呢，最后将自定义的解析器：UserDefinitionParser 再次回观。
