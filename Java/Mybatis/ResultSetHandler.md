...
ResultSetHandler是一个接口，提供了两个函数分别用来处理普通操作和存储过程的结果，源码如下：
...
ResultSetHandler的具体实现类是DefaultResultSetHandler，其实现的步骤就是将Statement执行后的结果集，按照Mapper文件中配置的ResultType或ResultMap来封装成对应的对象，最后将封装的对象返回即可，DefaultResultSetHandler源码如下：
