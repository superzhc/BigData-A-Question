...
Apache Kafka 是一个发布-订阅开源消息代理应用程序。

Kafka主要有以下几个组件：
- 主题：类似与数据库中的表的概念，存储一类的消息
- 生产者：向Kafka生产消息
- 消费者：从Kafka中消费消息
- Broker：Kafka集群中的每一个服务器都称为Broker

**消费者组**

Kafka中有一个独有的概念就是*消费者组*，一个消费者组可以包含多个消费者，每个消费者只会消费主体中的一部分消息，多个消费者组是相互独立的概念，每个消费者组都可以从主题中获取消息。
